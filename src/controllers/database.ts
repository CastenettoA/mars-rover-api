...
export class DatabaseController {
  ...
  static databaseFilePath = path.normalize(
    __dirname + "/.." + "/database/" + process.env.DATABASE_FILE_NAME
  );
  ...
  static get(keyNames: string[]) {
    try {
      ...
      let objValues = {};
      keyNames.forEach((el, index) => {
        // loop on keys and search in database data
        if (data["database"][el])
          // set property only if the key exist on the database
